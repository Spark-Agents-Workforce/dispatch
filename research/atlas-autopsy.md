...
**Purpose:** Document what went wrong with a real orchestrator so Dispatch doesn't repeat it

---

## What Atlas Was

Atlas was the original orchestrator for an 18-agent workforce. It sat in the `main` agent slot and handled all incoming messages from the user via multiple chat channels. It was supposed to route tasks to specialist agents.
...
**Impact:** The user was locked out during every work session. Couldn't send new instructions, couldn't course-correct, couldn't reach other agents. Average lockout: 2-5 minutes per task. Multiple lockouts per session.
...
Atlas loaded 4-6 files at every session start: SOUL.md, AGENTS.md, USER.md, MEMORY.md, and daily notes. Often 50KB+ of context before the user said a word. This caused:
...
- The coding agent would start project tasks with zero memory of prior work
- The trading agent would lose position tracking context
- The social media agent would lose thread continuity
...
When the user requested multi-agent work, Atlas would spawn one agent, wait for completion, then spawn the next. A 3-agent task that could finish in 3 minutes took 9 minutes.
...
Atlas would wait for all tasks to complete before reporting back. Even when one agent finished in 30 seconds, the user wouldn't know until the slowest agent finished.

**Root cause:** Batch mindset instead of streaming mindset.

### Failure 6: Ambiguity Paralysis

When a request was ambiguous, Atlas would spend 30-60 seconds trying to figure out the right interpretation instead of asking a quick clarifying question.

**Root cause:** Optimized for "look smart" instead of "be fast."
...
3. **sessions_send for named agents, sessions_spawn for disposable.** Per-agent documentation.
